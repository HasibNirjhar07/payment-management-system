...
const ClassCard: FC<ClassCardProps> = ({ year, active, enrolled, lastMonth }) => (
  <Link href={`/class-detail/HSC${year}`} passHref>
    <div className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow duration-200 cursor-pointer">
      ...
      <p className={`flex items-center ${lastMonth > 0 ? 'text-green-600' : 'text-red-600'}`}>
        Last Month: {lastMonth > 0 ? '▲' : '▼'}{Math.abs(lastMonth)}
      </p>
    </div>
  </Link>
);
